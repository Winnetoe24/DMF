...
interface ResponseError {
    ...
    Die Nachricht ist frei wählbar, sollte jedoch den Nutzer*innen erklären, wodurch der Fehler entstand.\\
...
\end{lstlisting}
    ...
    Damit bei der Verarbeitung der Nachricht der Typ der Parameter anhand der Methode bestimmt werden kann, enthält die Variable Params nur einen Wert
    von Typ `json.RawMessage'.
...
type CancelParams struct {
    ...
    \\\\
    Diese Funktion wird im Abschnitt \nameref{subsubsec:cancel-service} (\ref{subsubsec:cancel-service}) implementiert.
...
type MarkedString struct {
    ...
    \end{lstlisting}
    Die Implementierung des Hover-Effekts wird im Abschnitt \nameref{subsubsec:hover-service} (\ref{subsubsec:hover-service}) beschrieben.\\
...
const (
    ...
    \end{lstlisting}
    \\\\
    Die Implementierung der Faltbereiche wird im Abschnitt \nameref{subsubsec:folding} (\ref{subsubsec:folding}) beschrieben.\\
...
type SelectionRangeResult []SelectionRange
    \end{lstlisting}
    Die Implementierung der Auswahlbereiche wird im Abschnitt \nameref{subsubsec:selection-service} (\ref{subsubsec:selection-service}) beschrieben.\\
    ...
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    Die Implementierung der semantischen Tokens wird im Abschnitt \hyperref[subsubsec:semantic-service]{SemanticTokensService} (\ref{subsubsec:semantic-service}) beschrieben.\\
...
type DiagnosticRelatedInformation struct {
    ...
    \end{lstlisting}
    Die Implementierung für die Erstellung der Diagnosen wird im Abschnitt \nameref{subsubsec:diagnostics-service} (\ref{subsubsec:diagnostics-service}) beschrieben.\\
    ...
    \begin{lstlisting}[language=Go, caption=Definition des Connection-Interfaces]
package connect
type Connection interface {
    ...
    \end{center}
    \paragraph{JSON}\mbox{}\\
    Damit die Nachrichten übermittelt werden können, müssen sie ins JSON-Format geparsed werden.
...
err = json.Unmarshal(data, &message)
    ...
    Wird eine leere Zeile gelesen so wird die Schleife zum Auslesen des Headers beendet.\\
    Nun wird ein Buffer mit der gespeicherten Größe angelegt und mit dem Inhalt aus der Verbindung befüllt.
    Der Inhalt des Buffers kann jetzt als Text weiterverwendet werden.
    \\\\
...
func (s *Server) nextMethod(logger *log.Logger) bool {
    ...
    \begin{lstlisting}[language=Go, caption=Definition des MethodHandler-Interfaces]
package service
type MethodHandler interface {
    ...
    \end{lstlisting}
    Jeder Service implementiert die folgenden drei Methoden.
    \begin{enumerate}
    ...
    \begin{lstlisting}[language=Go, caption=Definition des FileServices]
package fileService
type FileService struct {
    ...
    \begin{lstlisting}[language=Go, caption=Definition des FileHandles]
package fileService
...
func createActionChannel() chan func() {
    ...
    \begin{lstlisting}[language=Go, caption=Laden eines FileHandles]
func (receiver *FileService) GetFileContent(uri protokoll.DocumentURI) (FileContent, error) {
    ...
    \begin{lstlisting}[language=Go, caption=Definition des FileChangeListeners]
package fileService
type FileChangeListener interface {
    ...
    \begin{center}
        \begin{longtable}{|c|c|>{\centering\arraybackslash}m{\mycolwidth}|}
            \hline
...
type semanticElement struct {
    ...
    \begin{lstlisting}[language=Go,caption=Sortieren der Token, label={lst:sortSemanticTokens}]
...
for {
    ...
    \begin{lstlisting}[caption=Die Query für alle Kommentare, label=lst:commentQuery]
(comment_block) @cb
    \end{lstlisting}
...
%    \vskip{-1em}
    \begin{table}[H]
        ...
        \begin{tabular}{|c|c|}
            \hline
            \textbf{\acrshort{ast}-Node Namen} & \textbf{Verwendung} \\
            \hline
...
%    \end{center}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|c|c|}
            \hline
            & \textbf{\acrshort{ast}-Node-Typen}\\
            \hline
            \textbf{Variable} & arg_block, ref_block, multi_block \\
            \hline
            \textbf{EntityIdentifier} & identifier_statement\\
            \hline
            \textbf{Konstante} & enum_constant \\
            \hline
            \textbf{Kommentar} & comment_block \\
            \hline
            \textbf{PackageElement} & \makecell{package_block, struct_block, enum_block,\\entity_block, interface_block} \\
            \hline
            \textbf{Import} & import_statement\\
            \hline
            \textbf{Model Deklaration} & model_declaration \\
            \hline
            \textbf{DMF Deklaration} & dmf_declaration \\
            \hline
        \end{tabular}
    \end{table}
    ...
    Die Standardbibliothek von Golang bietet eine Template-Engine, die für das Generieren von Texten genutzt wird.
...
return buffer.String()
    ...
    \\
    \textbf{Die Datenstrukturen} werden normal im Golang-Code definiert.
